...
import cn.bupt.edu.rejecthandler.ServerRejectedExecutionHandler;
...
public class ServerThreadPool {
    ...
    private static ExecutorService es = new ServerThreadPoolExecutor(2, 4, 10, SECONDS, new ArrayBlockingQueue<>(10), new TFactory(), new ServerRejectedExecutionHandler());
    ...
    public static void initThreadPool(String... servcie) {
        if (servcie.length > 0) {
            for (int i = 0; i < servcie.length; i++) {
                ...
                ExecutorService task = new ServerThreadPoolExecutor(2, 4, 10, SECONDS, tasks, new TFactory(), new ServerRejectedExecutionHandler());
